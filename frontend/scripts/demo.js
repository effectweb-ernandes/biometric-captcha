...
async function runBot(){
  ...
  addLog('BOT iniciado -- delay fixo 50ms','lb');
  ...
  addLog('BOT concluido -- CV: '+(mn(iv)>0?std(iv)/mn(iv):0).toFixed(3),'lb');
...
function enviar(){
  ...
  if(sc>=45){st.className='status ok';st.textContent='PASS ('+sc+'%) -- Comportamento humano confirmado!';}
